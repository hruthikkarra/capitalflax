...
const features = [
    'Same-day disbursal for urgent needs',
    'No collateral or guarantor required',
    'Use for any personal need (medical, travel, wedding)',
    'Flexible EMI options to suit your budget',
    'Online application and instant approval',
    'Minimal documentation for salaried professionals',
];
...
export default function PersonalLoanDetail() {
    ...
    const loanPct = ((loanAmount - 100000) / (5000000 - 100000)) * 100;
    const tenurePct = ((tenure - 1) / 4) * 100;
            ...
            <div style={{ maxWidth: '1200px', margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '5rem', alignItems: 'start' }}>
                ...
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '2.5rem' }}>
                        ...
                        <h2 style={{ fontSize: '2.25rem', fontWeight: 800, color: '#0F172A', margin: 0 }}>Personal Loans</h2>
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderTop: '1px solid #F1F5F9', borderLeft: '1px solid #F1F5F9', marginBottom: '2.5rem' }}>
                        {[
                            ...
                            { label: 'INTEREST RATE', value: 'Starting 10.5% p.a.' },
                            { label: 'TENURE', value: 'Up to 5 years' },
                            { label: 'PROCESSING FEE', value: '2%' },
                        ].map(s => (
                    ...
                    <Link href="#" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', color: '#10B981', fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none' }}>
                ...
                <div style={{ position: 'sticky', top: '90px' }}>
                    <div style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: '16px', padding: '2.5rem', boxShadow: '0 8px 32px -8px rgba(0,0,0,0.08)' }}>
                        ...
                        <div style={{ marginBottom: '2rem' }}>
                            ...
                            <input type="range" min={100000} max={5000000} step={50000} value={loanAmount}
                                onChange={e => setLoanAmount(Number(e.target.value))}
                                className="pers-slider" style={sliderStyle(loanPct)} />
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.4rem' }}>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>₹1L</span>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>₹50L</span>
                            </div>
                        </div>
                        ...
                        <div style={{ marginBottom: '2rem' }}>
                            ...
                            <input type="range" min={1} max={5} step={1} value={tenure}
                                onChange={e => setTenure(Number(e.target.value))}
                                className="pers-slider" style={sliderStyle(tenurePct)} />
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.4rem' }}>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>1 Year</span>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>5 Years</span>
                            </div>
                        </div>

                        {/* Interest Rate - Adjustable */}
                        <div style={{ background: '#F0FDF4', borderRadius: '8px', padding: '0.75rem 1rem', marginBottom: '0.5rem' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
                                <span style={{ fontSize: '0.85rem', color: '#065F46', fontWeight: 600 }}>Interest Rate</span>
                                <span style={{ fontSize: '0.9rem', fontWeight: 800, color: '#065F46' }}>{interestRate.toFixed(1)}% p.a.</span>
                            </div>
                            <input type="range" min={8} max={24} step={0.1} value={interestRate}
                                onChange={e => setInterestRate(Number(e.target.value))}
                                className="pers-slider" style={sliderStyle(ratePct)} />
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.4rem' }}>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>8%</span>
                                <span style={{ fontSize: '0.78rem', color: '#94A3B8' }}>24%</span>
                            </div>
                        </div>
                        <div style={{ marginBottom: '2rem' }}>
